...
def main():
    ...
    s.on_task_start(lambda n: print("[start]", n))
    s.on_task_done(lambda n, ok: print("[done ]", n, ok))
    s.on_scheduler_start(lambda info: print(f"Starting {info['total_tasks']} tasks across a {info['workers']} pool"))
    s.on_scheduler_done(lambda s: print(json.dumps(s, indent=2)))
    s.register(i01, 'i01')
    s.register(i02, 'i02')
    s.register(i03, 'i03')
    s.register(i04, 'i04')
